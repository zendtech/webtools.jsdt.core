 ...
 * Completion proposal.
 * <p>
 * In typical usage, the user working in a JavaScript code editor issues a
 * code assist command. This command results in a call to
 ...
 * analyzes the code in the buffer, determines what kind of JavaScript
 * language construct is at that position, and proposes ways to complete that
 * construct. These proposals are instances of the class
 * <code>CompletionProposal</code>. These proposals, perhaps after sorting and
 * filtering, are presented to the user to make a choice.
 * </p>
 * <p>
 ...
 * {@linkplain #getReplaceEnd() end}. The string can be arbitrary; for
 * example, it might include not only the name of a function but a set of
 ...
 * 
 *      Provisional API: This class/interface is part of an interim API that
 ...
 *      any code that uses this API will almost certainly be broken
...
public final class CompletionProposal extends InternalCompletionProposal {
	 ...
	 * <li>{@link #getDeclarationKey()} - the type unique key of the type
	 * being implemented or subclassed</li>
	 * <li>{@link #getSignature()} - the method signature of the constructor
	 * that is referenced</li>
	 * <li>{@link #getKey()} - the method unique key of the constructor that
	 * is referenced if the declaring type is not an interface</li>
	 * <li>{@link #getFlags()} - the modifiers flags of the constructor that
	 * is referenced</li>
	 * </ul>
	 * </p>
	 * 
	...
	/**
	 * Completion is a reference to a field. This kind of completion might
	 * occur in a context like <code>"this.ref^ = 0;"</code> and complete it
	 * to <code>"this.refcount = 0;"</code>.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 * <ul>
	 ...
	 * <li>{@link #getFlags()} - the modifiers flags of the field that is
	 * referenced</li>
	 * <li>{@link #getName()} - the simple name of the field that is
	 * referenced</li>
	 * <li>{@link #getSignature()} - the type signature of the field's type
	 * (as opposed to the signature of the type in which the referenced field
	 * is declared)</li>
	 * </ul>
	 * </p>
	 * 
	...
	/**
	 * Completion is a keyword. This kind of completion might occur in a
	 * context like <code>"fu Foo {}"</code> and complete it to
	...
	/**
	 * Completion is a reference to a label. This kind of completion might
	 * occur in a context like <code>"break lo^;"</code> and complete it to
	 * <code>"break loop;"</code>.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 * <ul>
	 * <li>{@link #getName()} - the simple name of the label that is
	 * referenced</li>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * of completion proposal at little extra cost:
	 * <ul>
	 * <li>{@link #getFlags()} - the modifiers flags of the local variable
	 * that is referenced</li>
	 * <li>{@link #getName()} - the simple name of the local variable that is
	 * referenced</li>
	 * <li>{@link #getSignature()} - the type signature of the local
	 * variable's type</li>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * occur in a context like <code>"myObject.pr^();"</code> and complete it
	 * to <code>""myObject.println();"</code>.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 * <ul>
	 ...
	 * referenced</li>
	 * <li>{@link #getSignature()} - the method signature of the method that
	 * is referenced</li>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * <li>{@link #getSignature()} - the method signature of the method that
	 * is being overridden or implemented</li>
	 ...
	 * the package that is referenced</li>
	 * </ul>
	 * </p>
	 * 
	 * <b>This completion only applies to ECMAScript 4 which is not yet
	 * supported</b>
	 * 
	 * @see #getKind()
	 */
	...
	/**
	 * Completion is a reference to a type. This kind of completion might
	 * occur in a context like <code>"var c=new Str^ ;"</code> and complete it
	 * to <code>"var c=new String ;"</code>.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 * <ul>
	 * <li>{@link #getDeclarationSignature()} -</li>
	 * <li>{@link #getSignature()} - the type signature of the type that is
	 * referenced</li>
	 * <li>{@link #getFlags()} - the modifiers flags (including
	 * Flags.AccInterface) of the type that is referenced</li>
	 * </ul>
	 * </p>
	 * 
	 * @see #getKind()
	...
	/**
	 * Completion is a declaration of a new potential function. This kind of
	...
	/**
	 * Completion is a link reference to a field in a JSdoc text. This kind of
	 * completion might occur in a context like
	...
	/**
	 * Completion is a link reference to a function in a JSdoc text. This kind
	 * of completion might occur in a context like
	...
	/**
	 * Completion is a link reference to a type in a JSdoc text. Any kind of
	 * type is allowed, including primitive types, reference types, array
	 * types, parameterized types, and type variables. This kind of completion
	 * might occur in a context like <code>"	* blabla Str^ blabla"</code> and
	 ...
	 * <li>{@link #getSignature()} - the type signature of the type that is
	 * referenced</li>
	 * <li>{@link #getFlags()} - the modifiers flags of the type that is
	 * referenced</li>
	 * </ul>
	 * </p>
	 * 
	...
	/**
	 * Completion is a method argument or a class/method type parameter in
	 * JSdoc param tag.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 ...
	 * </p>
	 * 
	 * <b>This field only applies to ECMAScript 4 which is not yet
	 * supported</b>
	 * 
	 * @see #getKind()
	 */
	...
	/**
	 * Completion is a JSdoc block tag. This kind of completion might occur in
	 * a context like <code>"	* @s^ blabla"</code> and complete it to
	...
	/**
	 * Completion is a JSdoc inline tag. This kind of completion might occur
	 * in a context like <code>"	* Insert @l^ Object"</code> and complete it
	 * to <code>"	* Insert {&#64;link Object }"</code>.
	 * <p>
	 * The following additional context information is available for this kind
	 * of completion proposal at little extra cost:
	 * <ul>
	 ...
	 * <ul>
	 * <li>{@link #getDeclarationSignature()} - the type signature of the type
	 * that declares the field that is imported</li>
	 * <li>{@link #getFlags()} - the modifiers flags (including ACC_ENUM) of
	 * the field that is imported</li>
	 ...
	 * <ul>
	 * <li>{@link #getDeclarationSignature()} - the type signature of the type
	 * that declares the method that is imported</li>
	 * <li>{@link #getFlags()} - the modifiers flags of the method that is
	 * imported</li>
	 * <li>{@link #getName()} - the simple name of the method that is imported
	 * </li>
	 * <li>{@link #getSignature()} - the method signature of the method that
	 * is imported</li>
	 * <li>{@link #getAdditionalFlags()} - the completion flags (including
	...
	/**
	 * Completion is an import of reference to a type. Only reference to
	 ...
	 * <ul>
	 * <li>{@link #getDeclarationSignature()} - the dot-based package name of
	 * the package that contains the type that is imported</li>
	 * <li>{@link #getSignature()} - the type signature of the type that is
	 * imported</li>
	 * <li>{@link #getFlags()} - the modifiers flags (including
	...
	/**
	 * Completion is a reference to a constructor. This kind of completion
	 ...
	 * <ul>
	 * <li>{@link #getDeclarationSignature()} - the type signature of the type
	 * that declares the constructor that is referenced</li>
	 * <li>{@link #getFlags()} - the modifiers flags of the constructor that
	 * is referenced</li>
	 * <li>{@link #getName()} - the simple name of the constructor that is
	 * referenced</li>
	 * <li>{@link #getSignature()} - the method signature of the constructor
	 * that is referenced</li>
	 * </ul>
	 * </p>
	 * <p>
	 * This kind of proposal could require a long computation, so they are
	 * computed only if completion operation is called with a
	 * {@link IProgressMonitor} (e.g.
	 * {@link ICodeAssist#codeComplete(int, CompletionRequestor, IProgressMonitor)}
	 * ).<br>
	 * This kind of proposal is always is only proposals with a
	 * {@link #TYPE_REF} required proposal, so this kind of required proposal
	 * must be allowed:
	 ...
	 * containing the relevant token defaults to empty subrange at [0,0).
	 */
	...
	/**
	 * Signature of the relevant package or type declaration in the context,
	 * or <code>null</code> if none. Defaults to null.
	 */
	private char[] declarationSignature = null;

	/**
	 * Unique key of the relevant package or type declaration in the context,
	 * or <code>null</code> if none. Defaults to null.
	 */
	...
	/**
	 * Signature of the function, field type, member type, relevant in the
	 * context, or <code>null</code> if none. Defaults to null.
	 */
	private char[] signature = null;

	/**
	 * Unique of the function, field type, member type, relevant in the
	 * context, or <code>null</code> if none. Defaults to null.
	 */
	 ...
	 * The proposal can not be applied if the required proposals aren't
	 * applied. Defaults to <code>null</code>.
	 */
	private CompletionProposal[] requiredProposals;

	/**
	 * Modifier flags relevant in the context, or
	 * <code>Flags.AccDefault</code> if none. Defaults to
	 ...
	 * <code>CompletionFlags.Default</code> if none. Defaults to
	...
	CompletionProposal(int kind, int completionLocation) {
		...
		if (this.completion == null || completionLocation < 0) {
			...
			// completionLocation can be -1 if the completion occur at the
			// start of a file or
			// the start of a code snippet but this API isn't design to
	 ...
	 * <p>
	 * This field is available for the following kinds of completion
	 * proposals:
	 * <ul>
	 * <li><code>FIELD_IMPORT</code> - completion flags of the attribute that
	 * is referenced. Completion flags for this proposal kind can only include
	 * <code>CompletionFlags.StaticImport</code></li>
	 * <li><code>METHOD_IMPORT</code> - completion flags of the attribute that
	 * is referenced. Completion flags for this proposal kind can only include
	 * <code>CompletionFlags.StaticImport</code></li>
	 * <li><code>TYPE_IMPORT</code> - completion flags of the attribute that
	 * is referenced. Completion flags for this proposal kind can only include
	 * <code>CompletionFlags.StaticImport</code></li>
	 * </ul>
	 ...
	 * </p>
	 * 
	 * @return the completion flags, or <code>CompletionFlags.Default</code>
	 *         if none
	 * @see CompletionFlags
	 * 
	 */
	 ...
	 * <p>
	 * The set of different kinds of completion proposals is expected to
	 * change over time. It is strongly recommended that clients do <b>not</b>
	 * assume that the kind is one of the ones they know about, and code
	 * defensively for the possibility of unexpected future growth.
	 * </p>
	 * 
	 ...
	 * file buffer containing the relevant token being completed. This token
	 * is either the identifier or JavaScript language keyword under, or
	 * immediately preceding, the original request offset. If the original
	 * request offset is not within or immediately after an identifier or
	 * keyword, then the position returned is original request offset and the
	...
	/**
	 * Returns the character index of the end (exclusive) of the subrange in
	 * the source file buffer containing the relevant token. When there is no
	 ...
	 * identifier or JavaScript language keyword under, or immediately
	 * preceding, the original request offset. If the original request offset
	 * is not within or immediately after an identifier or keyword, then the
	 * source range begins at original request offset and is empty.
	 * <p>
	 ...
	 * file buffer, replacing the characters at the specified source range.
	 * The string can be arbitrary; for example, it might include not only the
	 * name of a method but a set of parentheses.
	 * <p>
	...
	public char[] getCompletion() {
		if (this.completionKind == METHOD_DECLARATION) {
		...
		else if (this.completionKind == CONSTRUCTOR_INVOCATION) {
			this.findParameterNames(null);
			if (this.updateCompletion) {
				this.updateCompletion = false;
			...
			else if (new String(completion).equals("()")) {

				StringBuffer completionBuffer = new StringBuffer(this.completion.length);
				completion = completionBuffer.append(name).append(completion).toString().toCharArray();
			}
		}
	 ...
	 * string can be arbitrary; for example, it might include not only the
	 * name of a method but a set of parentheses.
	 * <p>
	 ...
	 * possible for a type completion to propose inserting an import
	 * declaration at the top of the compilation unit; or the completion might
	 * include trailing parentheses and punctuation for a method completion.
	 * </p>
	 * 
	...
	/**
	 * Returns the character index of the end of the subrange in the source
	 * file buffer to be replaced by the completion string. If the subrange is
	 * empty (<code>getReplaceEnd() == getReplaceStart()</code>), the
	 * completion string is to be inserted at this index.
	 * 
	...
	/**
	 * Returns the type signature of the relevant declaration in the context,
	 * or <code>null</code> if none.
	 * <p>
	 * This field is available for the following kinds of completion
	 * proposals:
	 * <ul>
	 * <li><code>ANNOTATION_ATTRIBUT_REF</code> - type signature of the
	 ...
	 * <li><code>FIELD_IMPORT</code> - type signature of the type that
	 * declares the field that is imported</li>
	 * <li><code>FIELD_REF</code> - type signature of the type that declares
	 * the field that is referenced</li>
	 * <li><code>METHOD_IMPORT</code> - type signature of the type that
	 * declares the method that is imported</li>
	 * <li><code>FUNCTION_REF</code> - type signature of the type that
	 * declares the method that is referenced</li>
	 * <li><code>FUNCTION_DECLARATION</code> - type signature of the type that
	 * declares the method that is being implemented or overridden</li>
	 * <li><code>PACKAGE_REF</code> - dot-based package name of the package
	 * that is referenced</li>
	 * <li><code>TYPE_IMPORT</code> - dot-based package name of the package
	 * containing the type that is imported</li>
	 * <li><code>TYPE_REF</code> - dot-based package name of the package
	 * containing the type that is referenced</li>
	 * <li><code>POTENTIAL_METHOD_DECLARATION</code> - type signature of the
	 * type that declares the method that is being created</li>
	 * </ul>
	 * For kinds of completion proposals, this method returns
	 * <code>null</code>. Clients must not modify the array returned.
	 * </p>
	 * 
	 * @return a type signature or a package name (depending on the kind of
	 *         completion), or <code>null</code> if none
	 * @see Signature
	 */
	 ...
	 * <li><code>FUNCTION_DECLARATION</code> - key of the type that declares
	 ...
	 * @see org.eclipse.wst.jsdt.core.dom.ASTParser#createASTs(IJavaScriptUnit[],
	 *      String[], org.eclipse.wst.jsdt.core.dom.ASTRequestor,
	 *      IProgressMonitor)
	 */
	...
	/**
	 * Sets the type signature of the relevant declaration in the context, or
	 * <code>null</code> if none.
	 * <p>
	 * If not set, defaults to none.
	 * </p>
	...
	/**
	 * Sets the type key of the relevant declaration in the context, or
	 * <code>null</code> if none.
	 * <p>
	 * If not set, defaults to none.
	 * </p>
	...
	/**
	 * Returns the simple name of the function, field, member, or variable
	 ...
	 * <li><code>FUNCTION_REF</code> - the name of the method (the type simple
	 * name for constructor)</li>
	 * <li><code>FUNCTION_DECLARATION</code> - the name of the method (the
	 * type simple name for constructor)</li>
	 ...
	 * field, member, or variable relevant in the context, or
	 * <code>null</code> if none.
	 * <p>
	 * If not set, defaults to none.
	 * </p>
	 ...
	 * 
	 * @param name
	 *            the keyword, field, method, local variable, or member name,
	 *            or <code>null</code> if none
	 */
	 ...
	 * <li><code>FIELD_IMPORT</code> - the type signature of the referenced
	 * field's type</li>
	 ...
	 * <li><code>METHOD_IMPORT</code> - method signature of the method that is
	 * imported</li>
	 * <li><code>FUNCTION_REF</code> - method signature of the method that is
	 * referenced</li>
	 * <li><code>FUNCTION_DECLARATION</code> - method signature of the method
	 * that is being implemented or overridden</li>
	 * <li><code>TYPE_IMPORT</code> - type signature of the type that is
	 * imported</li>
	 ...
	 * <li><code>POTENTIAL_METHOD_DECLARATION</code> - method signature of the
	 ...
	 * <li><code>FUNCTION_DECLARATION</code> - method key of the method that
	...
	/**
	 * Returns the type name of the relevant declaration in the context
	 * without the package fragment, or <code>null</code> if none.
	 * <p>
	 * This field is available for the following kinds of completion
	 * proposals:
	 * <ul>
	 ...
	 * <li><code>FIELD_REF</code> - the dot-based type name of the type that
	 * declares the field that is referenced or an anonymous type
	 * instantiation ("new X(){}") if it is an anonymous type</li>
	 * <li><code>FUNCTION_REF</code> - the dot-based type name of the type
	 * that declares the method that is referenced or an anonymous type
	 * instantiation ("new X(){}") if it is an anonymous type</li>
	 * <li><code>FUNCTION_DECLARATION</code> - the dot-based type name of the
	 * type that declares the method that is being implemented or overridden</li>
	 * </ul>
	 * For kinds of completion proposals, this method returns
	 * <code>null</code>. Clients must not modify the array returned.
	 * </p>
	 ...
	 * <li><code>FIELD_IMPORT</code> - modifier flags of the field that is
	 * imported.</li>
	 ...
	 * <li><code>KEYWORD</code> - modifier flag corresponding to the modifier
	 * keyword</li>
	 * <li><code>LOCAL_VARIABLE_REF</code> - modifier flags of the local
	 * variable that is referenced</li>
	 * <li><code>METHOD_IMPORT</code> - modifier flags of the method that is
	 * imported;</li>
	 * <li><code>FUNCTION_REF</code> - modifier flags of the method that is
	 * referenced; <code>Flags.AccAnnotation</code> can be used to recognize
	 * references to annotation type members</li>
	 * <li><code>FUNCTION_DECLARATION</code> - modifier flags for the method
	 * that is being implemented or overridden</li>
	 * <li><code>TYPE_IMPORT</code> - modifier flags of the type that is
	 * imported; <code>Flags.AccInterface</code> can be used to recognize
	...
	/**
	 * Returns the required completion proposals. The proposal can be apply
	 * only if these required completion proposals are also applied. If the
	 * required proposal aren't applied the completion could create completion
	 * problems.
	 * 
	 * <p>
	 * This field is available for the following kinds of completion
	 * proposals:
	 * <ul>
	 * <li><code>FIELD_REF</code> - The allowed required proposals for this
	 * kind are:
	 * <ul>
	 ...
	 * </ul>
	 * </li>
	 * <li><code>FUNCTION_REF</code> - The allowed required proposals for this
	 * kind are:
	 * <ul>
	 * <li><code>TYPE_REF</code></li>
	 * <li><code>TYPE_IMPORT</code></li>
	 ...
	 * </p>
	 * <p>
	 * Other kinds of required proposals will be returned in the future,
	 * therefore clients of this API must allow with
	 * {@link CompletionRequestor#setAllowsRequiredProposals(int, int, boolean)}
	 * only kinds which are in this list to avoid unexpected results in the
	 * future.
	 * </p>
	 * <p>
	 * A required completion proposal cannot have required completion
	 * proposals.
	 * </p>
	 * 
	 * @return the required completion proposals, or <code>null</code> if
	 *         none.
	 * 
	 * @see CompletionRequestor#setAllowsRequiredProposals(int, int,boolean)
	 * 
	...
	/**
	 * Sets the list of required completion proposals, or <code>null</code> if
	 * none.
	 * <p>
	 * If not set, defaults to none.
	 * </p>
	...
	/**
	 * Finds the method or function parameter names. This information is
	 * relevant to method reference (and method declaration proposals).
	...
	/**
	 * Sets the method or function parameter names. This information is
	 * relevant to method reference (and method declaration proposals).
	 * <p>
	 * The completion engine creates instances of this class and sets its
	 * properties; this method is not intended to be used by other clients.
	...
	/**
	 * @return parameter names for this proposal, or <code>null</code> if they
	 *         are not set
	 * 
	...
	/**
	 * @return type names of the parameters for this proposal, or
	 *         <code>null</code> none are set
	 * 
	...
	/**
	 * @return The fully qualified type name returned by the proposed function
	 *         or the declared type of the proposed variable or field
	 */
	 ...
	 * or {@link IAccessRule#K_NON_ACCESSIBLE}. By default this method return
	 ...
	 * proposals:
	 * <ul>
	 * <li><code>FUNCTION_REF</code> - return <code>true</code> if the
	 * referenced method is a constructor</li>
	 * <li><code>FUNCTION_DECLARATION</code> - return <code>true</code> if the
