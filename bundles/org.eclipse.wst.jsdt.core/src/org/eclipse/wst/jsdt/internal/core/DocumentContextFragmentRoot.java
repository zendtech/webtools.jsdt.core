...
public class DocumentContextFragmentRoot extends PackageFragmentRoot {
	...
	/*
	 * if user includes dojo.js check if dojo.js.uncompressed.js exists
	 * instead and replace with that.
	 */
	...
	class RestrictedDocumentBinding implements IRestrictedAccessBindingRequestor {
		...
		public boolean acceptBinding(int type, int modifiers, char[] packageName, char[] simpleTypeName, String path, AccessRestriction access) {
			if (path != null && exclude != null && path.compareTo(exclude) == 0)
				return false;
			...
			for (int i = 0; workingCopies != null && i < workingCopies.length; i++) {
				if (workingCopies[i].getPath().toString().equals(path)) {
					if (DEBUG)
						System.out.println("DocumentContextFragmentRoot ====>" + "REJECTING binding..\n\t" + new String(simpleTypeName) + " in " + path + "\n\tfor file " + fRelativeFile.toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					if (DEBUG)
						System.out.println("\tType is in WorkingCopies "); //$NON-NLS-1$
					return false;
				}
			}
			...
			// if(path.toLowerCase().indexOf(DOJO_COMPRESSED)>0 &&
			// (includeString.toLowerCase().indexOf(UNCOMPRESSED_DOJO)>0)) {
			// this.foundPaths.add(path);
			// return true;
			// }
			//
			...
			// if(Util.isSameResourceString(path, systemFiles[i]) || (new
			// Path(systemFiles[i])).isPrefixOf(new Path(path))) {
			//					if(DEBUG) System.out.println("DocumentContextFragmentRoot ====>" + "Accepting binding.. " + new String(simpleTypeName) + " in " + path + " \n\tfor file " + fRelativeFile.toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
			// this.foundPaths.add(path);
			// return true;
			// }
			...
			// if(DEBUG) {
			// for(int i = 0;includedFiles!=null &&
			// i<includedFiles.length;i++) {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.eclipse.wst.jsdt.internal.core.search.
		 * IRestrictedAccessBindingRequestor#getFoundPaths()
		 */
		 ...
		 * 
		 * @see org.eclipse.wst.jsdt.internal.core.search.
		 * IRestrictedAccessBindingRequestor#setExcludePath(java.lang.String)
		 */
	...
	public DocumentContextFragmentRoot(IJavaScriptProject project, IFile resourceRelativeFile, IPath resourceAbsolutePath, IPath webContext, IIncludePathEntry rawClassPath) {
		...
		// dirty = true;
		if (DEBUG)
			System.out.println("DocumentContextFragmentRoot ====>" + "Creating instance for total of:>>" + ++instances + "<<.  \n\tRelative file:" + fRelativeFile.toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	 ...
	 * 
	 * @see
	 * org.eclipse.wst.jsdt.internal.core.PackageFragmentRoot#getRawClasspathEntry
	 * ()
	 */
	...
	public void setIncludedFiles2(String[] fileNames) {
		...
		for (int i = 0; equals && i < newImports.size(); i++) {
			if (((String) newImports.get(i)).compareTo(includedFiles[i]) != 0)
				equals = false;
		}

		if (DEBUG)
			System.out.println("DocumentContextFragmentRoot ====>" + "Imports " + (equals ? "did NOT change" : "CHANGED:") + "\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
		if (DEBUG) {
		...
		if (equals)
			return;
		/*
		 * start try and expand the include paths from the library entries if
		 * necisary
		 */
		...
		// System.arraycopy(newImports, 0, this.includedFiles, 0,
		// arrayLength);
	...
	public void setIncludedFiles(String[] fileNames) {
		...
		for (int i = 0; i < fileNames.length; i++) {
			...
			// newTimestamps[arrayLength] = new
			// Long(importFile.lastModified());
		...
		for (int i = 0; equals && i < arrayLength; i++) {
			if (newImports[i].compareTo(includedFiles[i]) != 0)
				equals = false;
		}

		// this.includedFiles!=null && (newImports !=null) &&
		// this.includedFiles.length == arrayLength;
		...
		// System.arraycopy(newTimestamps, 0, this.timeStamps, 0,
		// arrayLength);
		dojoHack();
		updateClasspathIfNeeded();
	...
	private void dojoHack() {
		...
		for (int i = 0; i < includedFiles.length; i++) {
			...
			if (includeString != null && dojoIndex >= 0) {
				/*
				 * found dojo.js replace it with dojo.js.uncompressed.js if it
				 * exists
				 */
	...
	private boolean isExcluded(IJavaScriptProject javaProject, IResource resource) {
		...
		try {
			classpath = javaProject.getResolvedIncludepath(false);
		}
	...
	private void updateClasspathIfNeeded() {
		...
		for (int i = 0; i < includedFiles.length; i++) {
			...
			if (theFile == null || javaProject.isOnIncludepath(theFile))
				continue;
			if (isExcluded(javaProject, theFile)) continue;
	...
	public IPath resolveChildPath(String childPathString) {
		...
		/*
		 * relative paths: ./testfile.js are relative to file scope absolute
		 * paths: /scripts/file.js are relative to absolutePath, and must be
		 * made relative to this resource if the file does not exist in
		 * context root, the path is the absolute path on the filesystem.
		 */
		...
		switch (childPathString.charAt(0)) {
			...
			default :
				...
				// if(resolvedPath.toFile()!=null &&
				// resolvedPath.toFile().exists()) break;
	...
	public boolean equals(Object o) {
		...
		boolean equalRelativeFileAndIncludedFileLengths = (this.fRelativeFile != null && this.fRelativeFile.equals(other.fRelativeFile)) && this.includedFiles != null && (other.includedFiles != null) && this.includedFiles.length == other.includedFiles.length;
		...
		// if(!(this.timeStamps[i].equals( other.timeStamps[i] ) )) return
		// false;
		// }
	...
	public SearchableEnvironment newSearchableNameEnvironment(WorkingCopyOwner owner) throws JavaScriptModelException {
		/*
		 * previously restricted searchable environment to 'this'. But that
		 * removes library entries from search results so going back to global
		 * project
		 */
		SearchableEnvironment env = super.newSearchableNameEnvironment(owner);// new
																				// SearchableEnvironment((JavaProject)getJavaProject(),this,
																				// owner);
	...
	public NameLookup newNameLookup(IJavaScriptUnit[] workingCopies) throws JavaScriptModelException {
		...
		return lookup;
		// return ((LookupScopeElementInfo)getElementInfo()).newNameLookup(
		// workingCopies);
	}
	...
	public NameLookup newNameLookup(WorkingCopyOwner owner) throws JavaScriptModelException {
		...
		// IJavaScriptUnit[] workingCopies = owner == null ? null :
		// manager.getWorkingCopies(owner, true/*add primary WCs*/);
	...
	public File isValidImport(String importName) {
		...
		if (file.isFile()) {
			...
			IFile resolved = null;
			/*
			 * since eclipse throws an exception if it doesn't exists
			 * (contrary to its API) we have to catch it
			 */
			...
			if (exists)
				return new File(resolved.getLocation().toString());
		}
		return null;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < includedFiles.length; i++) {
			me.append(includedFiles[i] /*
										 * + "\t\t\t\t" +
										 * timeStamps[i].longValue()
										 */+ "\n"); //$NON-NLS-1$
		}
